...
def single_charge_pot(y, z):
    ...
    #
class subproc_pack(object):
	def __init__(self, carrier_3d, potential_3d, t_ox, rt_queue):
		self.carrier_3d = carrier_3d
		self.potential_3d = potential_3d
		self.t_ox = t_ox
		self.rt_queue =	rt_queue

def hopping_x_section(sp, x):
#        global hop_ini
#        global hop_finl
    print("Signal 1")
    #global carrier_3d
    ...
    y = 0
    while y < np.shape(sp.carrier_3d)[1]:
        z = sp.t_ox 
        while z < np.shape(sp.carrier_3d)[2]:
            if sp.carrier_3d[x, y, z] == 1:
                v, dirt = v_all_dirt(sp.carrier_3d, sp.potential_3d, x, y, z)
                if v.sum() > 0:
...
def hopping():  
    ...
    sp = subproc_pack(carrier_3d, potential_3d, t_ox, rt_queue)
    for xi in range(len_x):
    	i = 0
    	spl = []
    	while i < cores:
    		subproc = mp.Process(target=hopping_x_section, args=(sp, xi))
    		spl.append(subproc)
    		i += 1
    	for sp in spl:
    		sp.start()
    ...
    while not rt_queue.empty():
        ...
        if rt[0] < rt_min:
            rt_min = rt[0]
            hop_site = rt[1]
    hop_ini = np.array(hop_site)
    v_hop, dirt_hop = v_all_dirt(carrier_3d, potential_3d, \
                             hop_site[0], hop_site[1], hop_site[2])
